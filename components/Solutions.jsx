      ...
      <div className="container">
        <div style={{ textAlign: "center", maxWidth: 760, margin: "0 auto 48px" }}>
          ...
          <h2 style={{
            ...
            <span style={{ color: "var(--pascal-emerald)", fontWeight: 700 }}>Sovereign by Design.</span>
          </h2>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 28 }}>
              {s.kpi.map((row, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: 22, fontWeight: 700, color: "var(--pascal-emerald)", letterSpacing: "-0.02em" }}>{row.v}</span>
                </div>
              ))}
            </div>
      ...
      <div style={baseStyle}>
        <div style={{ background: "var(--pascal-paper-card)", border: "1px solid var(--pascal-line)", borderRadius: 14, padding: 22 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            ...
            <span style={{ padding: "6px 12px", borderRadius: 999, background: "var(--pascal-mint)", color: "var(--pascal-emerald)", fontSize: 12, fontWeight: 600 }}>LONG · High conf</span>
          </div>
          <svg viewBox="0 0 280 80" width="100%" height="80">
            <polyline fill="none" stroke="var(--pascal-emerald)" strokeWidth="2" points={ticks.map((v,i) => `${(i/(ticks.length-1))*280},${80-(v/max)*70}`).join(" ")} />
            <polygon fill="rgba(39,156,123,0.10)" points={`0,80 ${ticks.map((v,i) => `${(i/(ticks.length-1))*280},${80-(v/max)*70}`).join(" ")} 280,80`} />
          </svg>
        </div>
        <div style={{ background: "var(--pascal-paper-card)", border: "1px solid var(--pascal-line)", borderRadius: 14, padding: 18, display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {[{ l: "Earnings revisions", v: "+ ↑ 4 analysts" },{ l: "Insider transactions", v: "Net buy · $14M" },{ l: "Supply chain tone", v: "Improving" }].map(d => (
            <div key={d.l} style={{ display: "flex", justifyContent: "space-between", fontSize: 14 }}>
              ...
              <span style={{ color: "var(--pascal-emerald)", fontWeight: 600 }}>{d.v}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div style={baseStyle}>
        ...
        <div style={{ background: "var(--pascal-ink)", color: "#fff", borderRadius: 14, padding: 18, display: "flex", alignItems: "center", gap: 14 }}>
          <span style={{ fontSize: 36, fontWeight: 700, color: "var(--pascal-emerald)", letterSpacing: "-0.02em" }}>4.2 hrs</span>
      ...
      <div style={baseStyle}>
        <div style={{ background: "var(--pascal-paper-card)", border: "1px solid var(--pascal-line)", borderRadius: 14, padding: 22 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <i data-lucide="file-text" style={{ width: 18, height: 18, color: "var(--pascal-emerald)" }}/>
            <span style={{ fontWeight: 700 }}>Q3 IC Memo — Global Equity</span>
            <span style={{ marginLeft: "auto", fontSize: 11, padding: "3px 8px", background: "var(--pascal-mint)", color: "var(--pascal-emerald)", borderRadius: 999, fontWeight: 600 }}>Auto-draft</span>
          </div>
          <div style={{ fontSize: 13, color: "var(--pascal-fg-muted)", lineHeight: 1.7 }}>
            <p>The portfolio outperformed its benchmark by 240 bps in Q3, with positioning in semiconductors and select industrials driving the largest contribution<sup style={{ color: "var(--pascal-emerald)" }}>[1]</sup>...</p>
          </div>
  ...
  return (
    <div style={baseStyle}>
          ...
          <tbody>
            {[["SNOW","13.4x","+29%","58"],["DDOG","12.0x","+25%","65"],["MDB","8.9x","+22%","51"],["NET","12.5x","+28%","48"],["CRWD","17.1x","+33%","72"]].map(row => (
              <tr key={row[0]} style={{ borderTop: "1px solid var(--pascal-line)" }}>
                ...
                <td style={{ padding: "8px 0", color: "var(--pascal-emerald)", fontWeight: 600 }}>{row[2]}</td>
      ...
      <div style={{ background: "var(--pascal-paper-card)", border: "1px solid var(--pascal-line)", borderRadius: 14, padding: 18, display: "flex", alignItems: "center", gap: 12 }}>
        <i data-lucide="presentation" style={{ width: 22, height: 22, color: "var(--pascal-emerald)" }}/>
        <div style={{ flex: 1 }}>
        ...
        <span style={{ padding: "6px 12px", borderRadius: 999, background: "var(--pascal-mint)", color: "var(--pascal-emerald)", fontSize: 12, fontWeight: 600 }}>Ready</span>
      </div>
    </div>
  );
};
